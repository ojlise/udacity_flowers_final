        ...
        for inputs, labels in trainloader:
            ...
            if steps % print_every == 0:
                ...
                if best_accuracy < accuracy/len(validloader) and accuracy/len(validloader) > 0.6:
                    ...
                    path = input_args.save_dir         
                    print(path)
                    #save_checkpoint(model, path, epochs, model.classifier, optimizer, running_loss, train_data,arch)
                    save_checkpoint(model, train_data, path, best_accuracy, arch, hidden_units)
                    #save_checkpoint(model, path, epochs, classifier, optimizer, loss, train_data, arch)
